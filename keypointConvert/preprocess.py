...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    ...
    print(
        'subtract the center joint #6 (right shoulder in COCO 17), which is close to body center in pingpong movement')
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        if skeleton.sum() == 0:
            continue
        if len(zaxis) == 4:
            joint_top = (skeleton[0, 0, zaxis[0]] + skeleton[0, 0, zaxis[1]]) * np.array([0.5, 0.5, 0.5])
            joint_bottom = (skeleton[0, 0, zaxis[2]] + skeleton[0, 0, zaxis[3]]) * np.array([0.5, 0.5, 0.5])
        else:
